...
const getAll = function (data, callback) {
  ...
  tumblrClient.blogPosts(tumblrBlogName, tumblrOpts, (err, data) => {
    ...
    posts = posts.map(post =>
      tumblrHelper.prettyPrintPost(post, opts.language));
...
const setOpts = function (req, res, language) {
  const opts = {
    language: language, layout: config.layout, tumblr_on: config.tumblr.on, is_mi_grano_de_arena: config.is_mi_grano_de_arena, locals: res.locals, ga: config.google.ga,
  };
  return opts;
};
